...
double ISDist::compute_weight_mean()
{
    double sum = 0.0;
    for (int c = 0; c < num_clauses; c++)
        sum += clause_weight[c];
...
double ISDist::compute_weight_std(double mean)
{
    double sum_sq = 0.0;
    for (int c = 0; c < num_clauses; c++)
    {
...
void ISDist::dac_read_params()
{
    // Read parameter update from stdin
    // Format: h_inc s_inc smooth_probability soft_smooth_probability rwprob rdprob
    // Or "CONTINUE" to keep current params
    // Or "STOP" to terminate
    string cmd;
    cin >> cmd;
...
void ISDist::local_search_with_decimation(char *inputfile)
{
    ...
    for (tries = 1; tries < max_tries; ++tries)
    {
        ...
        unsigned int dac_flip_counter = 0; // counts flips since last checkpoint
            ...
            if (hard_unsat_nb == 0)
            {
                ...
                if (soft_unsat_weight < opt_unsat_weight)
                {
                    ...
                    if (opt_unsat_weight <= best_known || 0 == opt_unsat_weight)
                    {
                        ...
                        if (dac_mode)
                        {
                            // In DAC mode, emit final state and wait for STOP
                            dac_emit_state();
                            dac_read_params();
                        }
                        return;
