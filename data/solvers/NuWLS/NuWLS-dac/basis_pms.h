...
#ifdef _WIN32
#include <windows.h>
static LARGE_INTEGER _start_freq, _start_count;
static double get_runtime()
{
	LARGE_INTEGER now;
	QueryPerformanceCounter(&now);
	return (double)(now.QuadPart - _start_count.QuadPart) / (double)_start_freq.QuadPart;
}
static void start_timing()
{
	QueryPerformanceFrequency(&_start_freq);
...
class ISDist
{
  private:
	...
	// DAC checkpoint parameters
	int dac_mode;              // 0 = normal, 1 = DAC checkpoint mode
	int dac_checkpoint_interval; // flips between checkpoints
	...
	// DAC checkpoint functions
	void dac_emit_state();
	void dac_read_params();
	double compute_weight_mean();
	double compute_weight_std(double mean);
