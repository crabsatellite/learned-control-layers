...
#ifdef _WIN32
static LARGE_INTEGER start_time_val;
static LARGE_INTEGER perf_freq;
static double get_runtime()
{
	LARGE_INTEGER stop;
	QueryPerformanceCounter(&stop);
	return (double)(stop.QuadPart - start_time_val.QuadPart) / (double)perf_freq.QuadPart;
}
static void start_timing()
{
	QueryPerformanceFrequency(&perf_freq);
	QueryPerformanceCounter(&start_time_val);
}
#else
